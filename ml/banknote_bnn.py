# PyTorch smoke test from
# https://visualstudiomagazine.com/articles/2020/11/04/pytorch-training.aspx
...
class BanknoteDataset(T.utils.data.Dataset):
    def __init__(self, src_file, num_rows=None):
        ...
        self.y_data = T.tensor(all_data[:, 4], dtype=T.float32).to(device)
...
def main():
    ...
    # 6. make a prediction
    raw_inpt = np.array([[4.4, 1.8, -5.6, 3.2]], dtype=np.float32)
    norm_inpt = raw_inpt / 20
    unknown = T.tensor(norm_inpt, dtype=T.float32).to(device)
