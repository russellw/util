...
# that causes it to exit with error
# commandline arguments are the command for the prover,
...
def test(x):
    ...
    try:
        ...
        # failure is success
        # if the prover exits with an error
        # then we have successfully detected the error
        return 1
...
x = tuple(["&"] + xs)
if not test(x):
    raise Exception("Initial test failed to detect the error")
print(size(x))
...
if not test(x):
    raise Exception("Final test failed to detect the error")
